					...
					<!-- 
					 ...
					 -->
						...
						<input type="button" onclick="location.href='./boardList.jsp'"
							value="게시판" /> <input type="button"
							onclick="location.href='./dropbox.jsp'" value="자료실" /> <input
							type="button" onclick="location.href='./logout.jsp'" value="로그아웃" />
						...
						<table border="1px">
							...
							<c:choose>
								<c:when test="${empty listOfBoard}">
									<tr>
										<td colspan="5">조회하실 게시글이 존재하지 않습니다.</td>
									</tr>
								</c:when>
								<c:otherwise>
									<c:forEach items="${listOfBoard}" var="boardlist" begin="0"
										step="1" end="${listOfBoard.size() - 1}">
										<tr>
											<td>${boardlist.no}</td>
											<td><a href="boardView.jsp?no=${boardlist.no}">${boardlist.title}</a></td>
